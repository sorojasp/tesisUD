...
boolean connectToWifiNetwork(String wifiNetworkName, String password){
 ...
 boolean sendSensorData(String serverName, String endPoint, int port,  struct concentracionesSensores  cs, WiFiClient client, JsonObject& jsonobj, String params){
  ...
  //tutorial del Json en arduino https://randomnerdtutorials.com/decoding-and-encoding-json-with-arduino-or-esp8266/
     ...
     if (client.connect(serverName, port)) {
      ...
      Serial.flush();
      // Make a HTTP request:
      client.println(String("POST ") +  String(endPoint) + params + String(" HTTP/1.1"));
      client.println("Host: " + String(serverName));
      client.println("Content-Type: application/json");
      client.println("Connection: close");
      client.print("Content-Length: ");
      client.println(jsonobj.measureLength());
      client.println(); // end HTTP request header
      jsonobj.printTo(client);
